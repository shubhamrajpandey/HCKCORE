      ...
      <div className="max-w-6xl mx-auto flex flex-wrap justify-center gap-8 -mt-50">
        {cardItems.map((card, index) => {
          ...
          return (
            <motion.div
              key={index}
              className="w-[320px] h-[350px] bg-white border rounded-xl p-6 shadow-[8px_8px_0px_#000,9.5px_9.5px_0px_#ffffff] cursor-pointer"
              initial={{ scale: 1 }}
              animate={controls}
              style={{ transformOrigin: "top left" }}
              onHoverStart={() => {
                controls.start({
                  scale: [1, 1.05, 1.025, 1.045, 1.03],
                  transition: {
                    type: "tween",
                    duration: 0.6,
                    ease: "easeOut",
              ...
              onHoverEnd={async () => {
                ...
                await controls.start({
                  scale: [1, 1.02, 1.01, 1.02, 1.01],
                  transition: { duration: 0.6, ease: "easeOut" },
                });
              }}
            >
              <div className="mb-3 border-none bg-[#E3E3E3] rounded-4xl h-10 w-10 p-1">
                {card.icon}
              </div>
              <motion.h4
