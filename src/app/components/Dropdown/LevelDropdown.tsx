...
export default function LevelDropdown() {
  const options = ["Level 4", "Level 5", "Level 6"];
  const [isOpen, setIsOpen] = useState(false);
  const [selectedLevel, setselectedLevel] = useState(options[0]);

  return (
    <div className="relative inline-block w-[180px] h-fit rounded-md">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="group flex items-center justify-between gap-3 px-4 py-2 bg-white border border-gray-300 rounded shadow w-[180px] h-16 hover:text-white hover:bg-[#74BF44] hover:brightness-90 transition duration-200 hover:cursor-pointer"
      >
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute mt-2 w-[180px] bg-white border border-gray-300 rounded shadow-md z-10">
